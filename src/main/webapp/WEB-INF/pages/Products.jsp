...
<form method="GET" style="text-align: center" action="<c:url value="/customer/previousOrders"/>">
    <input class="btn-danger" type="submit" value="Your Orders" name="previousOrders" id="previousOrders"/>
</form>

<form method="POST" style="text-align: center" action="<c:url value="/customer/addToCart"/>">
        ...
        <tbody>
            <c:forEach items="${requestScope.allProducts}" var="product">
                <tr>
                    ...
                    <td width="80px">
                        <a href="<c:url value="${requestScope.servletPath}/customer/addToCart"><c:param name="productId" value="${product.productId}"/></c:url>">Add To Cart</a>
                    </td>
                    <td width="80px">
                        <a href="<c:url value="${requestScope.servletPath}/customer/addToWishList"><c:param name="productId" value="${product.productId}"/></c:url>">Add To Wishlist</a>
                    </td>
                </tr>
        </c:forEach>
        <br/>
